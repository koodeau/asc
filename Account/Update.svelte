<script>
  ...
  const actions = {
    name: async name => {
      try {
        const response = await Appwrite.account.updateName(name);
    ...
    email: async (email, password) => {
      try {
        const response = await Appwrite.account.updateEmail(
          email,
          password
        );
    ...
    password: async (password, oldPassword) => {
      try {
        const response = await Appwrite.account.updatePassword(
          password,
          oldPassword
        );
